"""Сервис полинга рейтингов АКРА (тонкий провайдер на общем ядре)."""
...
from . import config
from .client import AcraClient


class AcraRatingPoller(BaseRatingPoller):
    """Провайдер АКРА: опрашивает acra-ratings.ru, остальное — общее ядро."""

    agency = RatingAgency.ACRA

    async def _poll(
        self, known: dict[str, str | None]
    ) -> tuple[list[RatingEvent], dict[str, ChangeType]]:
        """Опрашивает АКРА и возвращает новые/изменённые события с их типом."""
        async with AcraClient() as client:
