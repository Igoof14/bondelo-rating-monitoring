"""Асинхронный клиент к сайту «Эксперт РА» (raexpert.ru)."""
...
class RaClient:
    """Клиент для перечисления и загрузки релизов рейтингов «Эксперт РА»."""
    ...
    def _client(self) -> aiohttp.ClientSession:
        if self._session is None:
            raise RuntimeError("Клиент не инициализирован. Используйте 'async with RaClient()'")
        return self._session

    async def iter_release_stubs(self, max_items: int = config.MAX_ITEMS) -> list[ReleaseStub]:
        """Возвращает верхние ``max_items`` строк таблицы рейтингов (новейшие сверху)."""
        ...
        for _ in range(len(self._rotator.proxies)):
            ...
            try:
                ...
                last_error = e
                logger.warning(f"Листинг «Эксперт РА» через прокси {proxy} не удался: {e}")

        logger.error(f"Не удалось загрузить листинг «Эксперт РА»: {last_error}")
        return []
    ...
    async def fetch_release(self, stub: ReleaseStub) -> RatingEvent | None:
        ...
        Каждая попытка идёт через следующий прокси: 429 от QRATOR привязан к IP,
